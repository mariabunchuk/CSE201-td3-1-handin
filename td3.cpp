...
double* remove_from_array(double* array,
  ...
  if (max_size - current_size>=5) max_size = max_size - 5;
  return array;
}
...
void merge_telemetry(double **telemetries,
  ...
  {for (int i = 3; i < global_telemetry_current_size - 1; i+=3)
      if (global_telemetry[i] < global_telemetry[i-3]){
          double x = global_telemetry[i];
          global_telemetry[i] = global_telemetry[i-3];
          global_telemetry[i-3] = x;
          double y = global_telemetry[i-2];
          global_telemetry[i-2] = global_telemetry[i+1];
          global_telemetry[i+1] = y;
          double z = global_telemetry[i-1];
          global_telemetry[i-1] = global_telemetry[i+2];
          global_telemetry[i+2] = z;
      }}
